...
> Recrea el comportamiento del antiguo `/exit` que se perdió. Complementa a `/session-save`
> con foco en el roadmap vivo (`.savia-memory/sessions/ROADMAP.md`) y el RESUME de continuidad.
...
> NO hacer commit autónomo (Rule #25 + autonomous-safety: nada se mergea sin /pr-plan).
...
- `/exit` **NO** cierra OpenCode. Es un comando de persistencia, no de proceso.
- Si OpenCode/Windows se cuelga ANTES de invocar `/exit`, el `ROADMAP.md` (actualizado durante la sesión) ya tiene el estado mínimo recuperable.
