...
WARN_LOG="$PROJECT_DIR/output/data-sovereignty-warnings.log"
mkdir -p "$(dirname "$AUDIT_LOG")" 2>/dev/null

# Emit non-blocking WARNINGs to a log file, NOT stderr.
# Rationale: OpenCode v1.14 TUI concatenates hook stderr with frontend render
# without newline separators, causing visual overlap. Blocking errors (exit 2)
# MUST stay on stderr — OpenCode needs them to show the block reason.
shield_warn() {
  printf '[%s] %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || echo unknown)" "$1" \
    >> "$WARN_LOG" 2>/dev/null || true
}

# SHIELD-GATE-STDOUT-01: emit BLOCK decision as JSON to stdout so OpenCode/Claude
# renders the reason inline in the conversation instead of pinting raw stderr
# over the TUI (which leaves the screen visually broken). Protocol:
# PreToolUse hookSpecificOutput.permissionDecision="deny" + reason. Exit 0:
# the decision rides in the JSON, not the exit code. Falls back to stderr+exit 2
# only if jq is missing (defensive — jq is required elsewhere in this script).
shield_block_inline() {
  local reason_short="$1"   # one-line user-facing summary
  local reason_detail="$2"  # full detail (entities list, file path, etc.)
  local full_reason
  full_reason=$(printf '[Savia Shield] %s\n%s' "$reason_short" "$reason_detail")
  ...
  if [[ -n "$RESULT" ]]; then
    if echo "$RESULT" | grep -q '"BLOCK"'; then
      ...
      if [[ "$SH01_ALLOW" == "true" ]]; then
        shield_warn "Savia Shield SH01: code-token allowlist override en $FILE_PATH"
      ...
      ENT_LIST=$(echo "$RESULT" | jq -r '.entities[]? | "  [\(.type)] \(.text)"' 2>/dev/null | head -5)
      shield_block_inline \
        "PII detectado en fichero publico: $FILE_PATH" \
        "${ENT_LIST:-(sin entidades enumeradas)}"
    fi
    exit 0
  fi
fi
...
block_fallback() {
  ...
  shield_block_inline \
    "BLOQUEADO [fallback]: ${reason} en $FILE_PATH" \
    "Capa fallback (daemon caido). Detalle en output/data-sovereignty-audit.jsonl"
}
  ...
  case "$VERDICT" in
    CONFIDENTIAL)
      ...
      if [[ "$IS_N1_DEST" == "true" ]]; then
        ...
        shield_warn "Savia Shield: classifier AMBIGUOUS en $FILE_PATH (N1 dest, permitido)"
