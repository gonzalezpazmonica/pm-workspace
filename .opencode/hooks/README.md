...
65 hooks `.sh` protegidos contra vendor lock-in. Compartidos cross-frontend:
Claude Code los registra vía `.claude/settings.json` (stdin JSON), OpenCode
los ejecuta vía bridge `savia-gates` (Bun shell API + inyección de env vars).
Los 5 hooks Tier-1 tienen además ports TypeScript nativos en
`.opencode/plugins/guards/` para latencia mínima bajo OpenCode.
