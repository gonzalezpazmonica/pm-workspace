...
if [[ -n "$LEAK" ]]; then
  ...
  # SHIELD-AUDIT-STDOUT-01: NO escribir a stderr — pisa la TUI de OpenCode/Claude.
  # La alerta queda en el JSONL append-only auditado por humanos:
  #   tail -n 5 output/data-sovereignty-audit.jsonl
  # Si necesitas notificación inmediata, usa el plugin TS savia-foundation que
  # canaliza vía notification API en lugar de stderr crudo.
fi
