...
2. **Catálogo** — muestra los comandos por categoría (nombre, params, descripción breve):
   ...
   **Conectores (7):** notify:slack {canal} {msg}, slack:search {query}, sentry:health --project {p}, sentry:bugs --project {p}, gdrive:upload {file} --project {p}, linear:sync --project {p}, notion:sync --project {p}
   **Utilidades (2):** context:load, help [filtro]

   Si $ARGUMENTS filtra (sprint, pbi, sdd, pr, team, infra, diagram, slack, sentry, gdrive, linear, notion, connectors, --setup), mostrar solo esa sección.
