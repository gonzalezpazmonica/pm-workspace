...
print("# docs/rules/domain — INDEX")
print()
print(f"> Auto-generated index of all {len(files)} rule files. Consult this before creating a new rule to avoid drift and duplication.")
print()
print(f"**Total**: {len(files)} files across {len(cats)} categories.")
print()
print("Regenerate with: `bash scripts/rules-domain-index.sh`. CI checks freshness with `--check`.")
print()
...
for cat in sorted(cats.keys()):
    ...
    for fname, desc in sorted(cats[cat]):
        print(f"- **[`{fname}`](./{fname})** — {desc}")
    print()

print()
print("---")
print()
print("*Generated by `scripts/rules-domain-index.sh` as part of SPEC-115.*")
PY
