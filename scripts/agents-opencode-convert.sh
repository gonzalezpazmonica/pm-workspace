...
case "$MODE" in
  ...
  check)
    while IFS= read -r src; do
      ...
      if ! diff -qB <(printf '%s' "$converted" | grep -vE '^(maxSteps|maxTurns|permission\.task|  allowlist):' | sed -e '$a\') <(grep -vE '^(maxSteps|maxTurns|permission\.task|  allowlist):' "$target" | sed -e '$a\') >/dev/null 2>&1; then
