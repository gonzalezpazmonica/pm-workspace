...
Sits between OpenCode/Claude Code and LLM API (DeepSeek by default).
Intercepts ALL outbound prompts, scans for sensitive data, masks entities,
forwards sanitized request, unmasks the response.

Usage:
  python3 scripts/savia-shield-proxy.py [--port 8443] [--target https://api.deepseek.com]
  Then (Claude Code): export ANTHROPIC_BASE_URL=http://127.0.0.1:8443
  Or (OpenCode): set provider.deepseek.options.baseURL in opencode.json
...
TARGET_URL = os.environ.get(
    "SAVIA_API_UPSTREAM", "https://api.deepseek.com"
)
...
def mask_text(text):
    ...
    for real_term in sorted_terms:
        # Lookarounds: only alphanumeric blocks the match (same as
        # sovereignty-mask.py). Sentence punctuation and path separators
        # (`.`, `_`, `-`, `/`) do NOT block, so paths like
        # "projects/acme-project_main/" still get masked.
...
def _mask_in_obj(obj, stats):
    ...
    Bug fix 2026-04-28: previous code only masked `messages[].content[].text`
    and skipped `system` unless it contained a credential. Real names in the
    system prompt (gitStatus, CLAUDE.md, environment) and in tool_result
    blocks (Bash/Read output) leaked unmasked to the upstream API.
    """
...
def process_response_body(body_bytes, content_type=None):
    ...
    # tool_use inputs, thinking blocks, etc. (Bug fix: previous code only
    # unmasked top-level content[].text, missing many fields.)
    _unmask_in_obj(body)
