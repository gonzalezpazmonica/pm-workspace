...
H = Path.home()
PORT = 9221
SIG = H / ".savia" / "corp-extract.signal"
STOP = H / ".savia" / "corp-stop.signal"
CALSIG = H / ".savia" / "corp-calendar.signal"
...
c = p.chromium.launch_persistent_context(
    str(H/".savia"/"chromium-corp"), headless=False,
...
print(f"CORP_DAEMON_READY port={PORT}", flush=True)
...
def ex_cal():
    ...
    pg.evaluate("""() => {
        document.querySelectorAll('button').forEach(b => {
            ...
            if (al.includes('ir al dia siguiente')) b.click();
        });
    }""")
    pg.wait_for_timeout(5000)
    return pg.evaluate("""() => {
        ...
        document.querySelectorAll('[aria-label]').forEach(el => {
            ...
            if (!al.match(/[0-9]{1,2}[:.][ 0-9]{2}/)) return;
    ...
    if SIG.exists():
        ...
        try:
            ...
            (OUT / "inbox-corp-chromium.json").write_text(
                ...
                encoding="utf-8")
            print(f"CORP INBOX: {len(em)}", flush=True)
        except Exception as e: print(f"err in: {e}", flush=True)
    if CALSIG.exists():
        CALSIG.unlink()
        try:
            evs = ex_cal()
            (OUT / "cal-corp-15.json").write_text(
                json.dumps({"account":"corp-account","ts":time.strftime("%Y-%m-%dT%H:%M:%S"),"count":len(evs),"events":evs}, ensure_ascii=False, indent=2),
                encoding="utf-8")
            print(f"CORP CAL: {len(evs)}", flush=True)
        except Exception as e: print(f"err cal: {e}", flush=True)
c.close(); p.stop()
print("CORP_STOPPED", flush=True)
