...
@test "naming-only routing output is exactly 'cognitive-judge'" {
  local f; f=$(make_findings '{"detail": "poor naming: variable x has no semantic meaning"}')
  run bash "$SCRIPT" --findings "$f" --round 1 --max-round 5
  [ "$status" -eq 0 ]
  [ "$output" = "cognitive-judge" ]
}

@test "--round abc -> exit 2, stderr contains 'integer'" {
  local f; f=$(make_findings '{"detail": "sql injection in login"}')
  run bash "$SCRIPT" --findings "$f" --round abc --max-round 3
  [ "$status" -eq 2 ]
  [[ "$output" == *"integer"* ]] || [[ "${lines[*]}" == *"integer"* ]]
}

@test "findings with keyword 'dod' -> spec-judge + correctness-judge" {
  local f; f=$(make_findings '{"detail": "dod not met: AC-02 missing implementation"}')
  run bash "$SCRIPT" --findings "$f" --round 1 --max-round 5
  [ "$status" -eq 0 ]
  echo "$output" | grep -q "spec-judge"
  echo "$output" | grep -q "correctness-judge"
...
@test "findings with keyword 'solid' -> architecture-judge + spec-judge" {
  local f; f=$(make_findings '{"detail": "solid open-closed principle violation in service layer"}')
  run bash "$SCRIPT" --findings "$f" --round 1 --max-round 5
  [ "$status" -eq 0 ]
  echo "$output" | grep -q "architecture-judge"
  echo "$output" | grep -q "spec-judge"
...
@test "round=4 max-round=5 (last-round override) returns all 5 judges" {
  local f; f=$(make_findings '{"detail": "xss vulnerability in form"}')
  run bash "$SCRIPT" --findings "$f" --round 4 --max-round 5
  [ "$status" -eq 0 ]
  local got; got=$(sorted_judges "$output")
  [ "$got" = "$ALL_JUDGES_SORTED" ]
}

@test "round=4 max-round=9 (no last-round override) returns reduced set" {
  local f; f=$(make_findings '{"detail": "xss vulnerability in form"}')
  run bash "$SCRIPT" --findings "$f" --round 4 --max-round 9
  [ "$status" -eq 0 ]
  # round 4 < max-round-1 (8): should not trigger last-round override
  local count; count=$(echo "$output" | sort -u | wc -l)
  [ "$count" -lt 5 ]
}
