...
Incluye siempre: versión de Claude Code / OpenCode usada (`claude --version` o `opencode --version`), qué comando o skill está involucrado, qué comportamiento esperabas y qué obtienes, y si es reproducible con el proyecto de test `sala-reservas` en modo mock.
