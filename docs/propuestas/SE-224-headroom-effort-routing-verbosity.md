...
slice_2_status: PENDING
slice_3_status: DEFERRED
note: Slice 2 (effort-router hook) pendiente hasta merge PR #853 (branch agent/se224-verbosity-sentinel-20260624)
---
...
### Slice 1 — Verbosity sentinel en cola system prompt (XS, 1h) — IMPLEMENTED 2026-06-24

- Hook `output-verbosity-sentinel.sh` (PostToolUse) creado en `.opencode/hooks/`
- Clasifica turns como MECHANICAL / ERROR / NEW_ASK
- Emite `<!-- VERBOSITY_LEVEL:L2 -->` como `hookSpecificOutput` en stderr (JSON)
- `caveman-default.md` actualizado con sección `## Verbosity Sentinel (SE-224)`
- BATS: `tests/test-se224-verbosity-sentinel.bats` (≥8 tests)
- Criterio: prefix cache no se invalida (instrucción en cola, no cabeza)

### Slice 2 — Effort router hook (S, 2h) — PENDING (post-merge PR #853)
