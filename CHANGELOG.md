...
**Era 232** — Shield NER: allowlist de dominios públicos y filtros estructurales contra falsos positivos.

### Fixed
- **Shield NER false positives en dominios públicos**: URLs de `*.microsoft.com`, `*.office.com`, `*.sharepoint.com`, `*.office365.com`, `*.microsoft365.com`, `outlook.office.com`, `stream.office.com`, `login.microsoftonline.com`, `*.google.com`, `github.com`, `gitlab.com`, `dev.azure.com` y similares dejaban de ser clasificadas como PII al escribir scripts legítimos. Se añade filtro `0a` en `scan()` con substring match case-insensitive contra `NER_PUBLIC_DOMAINS`.
- **Shield NER false positives en código**: identificadores `UPPER_SNAKE_CASE`, referencias dotted (`sys.path.insert`, `page.evaluate`), fragmentos de path (con `/`, `\` o extensiones `.py/.md/.js/.sh/.json/...`) y paths del propio workspace (`OneDrive - *`, `/Documentos/`, `/Documents/`) ya no disparan BLOCK. Filtros `0b`, `0c`, `0d` añadidos.

### Added
- `scripts/shield-ner-allowlist.txt`: 24 dominios públicos + 3 path markers del workspace documentados como referencia.
- `tests/test-shield-ner-allowlist-domains.bats`: 15 tests de regresión (8 positivos confirmando que dominios públicos, constantes y paths no se bloquean; 3 negativos confirmando que credenciales reales, GitHub PAT e IPs privadas siguen bloqueando; 4 sanidad del código fuente y allowlist).

### Changed
- `scripts/savia-shield-daemon.py`: constantes `NER_PUBLIC_DOMAINS` y `NER_PATH_EXTENSIONS` añadidas. Función `scan()` extendida con 4 filtros (0a–0d) antes del allow-list lookup existente.
