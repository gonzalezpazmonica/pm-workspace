...
### C1. OK TrustAllCertificates — Man-in-the-Middle
...
### C2. OK Base de datos Room sin cifrar
...
### C6. OK HttpLoggingInterceptor en producción
...
### A11. OK Encoding passphrase SQLCipher incorrecto
...
### C3. OK Inyección de comandos en PUT /git-config
...
### C4. OK GitHub PAT almacenado en Base64 (no cifrado)
...
### C5. OK Endpoints sensibles sin autenticación
...
### C7. OK Credenciales hardcodeadas en docker-compose.yml
...
### C11. OK Secretos K8s en texto plano YAML
...
### C12. OK CORS `AllowAnyOrigin()` en API .NET
...
### C13. OK JWT Secret placeholder en appsettings.json
...
### C10. OK `bash -c "$1"` en test scripts
...
### C14. OK `curl | sh` sin verificación
...
### C15. OK Trap con variable sin comillas → rm de paths arbitrarios
...
### C16. OK Grep fallback para JSON parsing en hooks
...
|---|--------|----------|-----------|
| M1 | OK | YAML frontmatter injection en Bridge | savia-bridge.py |
| M2 | OK | Session ID sin validación de formato | savia-bridge.py |
| M3 | OK | TLS sin cipher suite mínima | savia-bridge.py |
| M4 | OK | Cleartext traffic en debug | network_security_config.xml |
| M5 | OK | Temp files sin cleanup trap | scripts/add-maturity-levels.sh |
| M6 | OK | BATS clonado sin verificación | .github/workflows/ci.yml |
| M7 | OK | Secret regex loose en CI | .github/workflows/ci.yml |
| M8 | OK | Variables sin comillas en test scripts | scripts/test-*.sh |
| M9 | OK | Worker pod sin health checks | k8s/worker-deployment.yaml |
| M10 | OK | Secrets como env vars (TODO documentado) | k8s/api-deployment.yaml |
| M11 | OK | Docker frontend sin `npm ci --production` | frontend/Dockerfile |
| M12 | OK | JWT silent failures (bare catch) | JwtService.cs |
| M13 | OK | Sin policy de rotación de secretos | SECRETS-ROTATION.md |
| M14 | OK | appsettings.Production.json con templates | appsettings.Production.json |
| M15 | OK | Hook plan-gate.sh sin timeout explícito | .opencode/hooks/plan-gate.sh |
...
**Total hallazgos: 55** (18 críticos, 22 altos, 15 medios) — **55/55 resueltos OK**
